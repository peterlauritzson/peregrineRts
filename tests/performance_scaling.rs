...
impl SystemMetrics {
    fn total_ms(&self) -> f32 {
        self.spatial_hash_ms + self.boids_steering_ms + 
...
fn timed_spatial_hash(world: &mut World) {
    let t = Instant::now();
    // Use sequential spatial hash update
...
fn run_perf_test(config: PerfTestConfig) -> PerfTestResult {
    ...
    app.add_systems(Update, timed_boids_steering.after(timed_spatial_hash));
    ...
    while tick_count < target_ticks {
        ...
        if investigate_spatial_hash {
            ...
            let mut query = world.query::<(Entity, &OccupiedCell)>();
            for (entity, _occupied_cell) in query.iter(world) {
                entity_cell_history.entry(entity)
                    .or_insert_with(Vec::new)
                    .push(1);  // Each entity now occupies exactly one cell
            }
        }
    ...
    if investigate_spatial_hash && !entity_cell_history.is_empty() {
        ...
        if !entities_with_growth.is_empty() {
            ...
            if let Some(&sample_entity) = entities_with_growth.first().map(|(e, _, _, _)| e) {
                let mut query = world.query::<(Entity, &SimPosition, &Collider, &OccupiedCell)>();
                if let Some((_, pos, collider, occupied)) = query.iter(world).find(|(e, _, _, _)| *e == sample_entity) {
                    ...
                    println!("    Occupies cell: size_class={}, grid={}, col={}, row={}", 
                        occupied.size_class, 
                        if occupied.grid_offset == 1 { "B" } else { "A" },
                        occupied.col, 
                        occupied.row);
                }
            }
...
fn run_spatial_hash_diagnostics(app: &mut App, unit_count: usize, obstacle_count: usize) {
    ...
    // Note: The new staggered multi-resolution spatial hash design uses internal
    // cell storage that isn't exposed through a public API. Detailed cell diagnostics
    // are no longer available. For debugging, use the OccupiedCell component which
    // shows which cell each entity occupies.
    
    // TEST 1: Entity Coverage Check
    let total_entries: usize;
    {
        let world = app.world_mut();
        let mut query = world.query::<(Entity, &OccupiedCell)>();
        total_entries = query.iter(world).count();
    }
    
    let expected = unit_count + obstacle_count;
    
    println!("TEST 1: ENTITY COVERAGE");
    println!("  Entities with OccupiedCell: {}", total_entries);
    println!("  Expected entities: {}", expected);
    if total_entries == expected {
        println!("  ✓ All entities properly registered in spatial hash");
    } else {
        println!("  ⚠ WARNING: {} entities missing from spatial hash!", expected - total_entries);
    }
    ...
    if total_entries != expected {
        println!("  ⚠ Some entities missing OccupiedCell component!");
    } else {
        println!("  ✓ All entities properly tracked in spatial hash");
    }
