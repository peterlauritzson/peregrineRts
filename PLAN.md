...
- **Fixed Timestep**: The simulation will run at a fixed tick rate (e.g., 16-20 ticks per second), independent of the frame rate.
...
3. **The Box (Map Borders)**
   ...
   - Units stop or slide when hitting the edge of the map.

4. **Basic Unit-Unit Collision (Detection)**
   - Implement spatial hashing or simple N^2 check (for low count).
   - Detect when units overlap.
   - Visual debug indication of overlap.

5. **Basic Unit-Unit Collision (Resolution)**
   - Units push each other apart.
